...
func initConstantEnv() {
	...
	constant.ClaudeClientGoneDrainTimeout = GetEnvOrDefault("CLAUDE_CLIENT_GONE_DRAIN_TIMEOUT", 60)
	constant.DifyDebug = GetEnvOrDefaultBool("DIFY_DEBUG", true)
	constant.MaxFileDownloadMB = GetEnvOrDefault("MAX_FILE_DOWNLOAD_MB", 64)
	constant.StreamScannerMaxBufferMB = GetEnvOrDefault("STREAM_SCANNER_MAX_BUFFER_MB", 128)
	// MaxRequestBodyMB 请求体最大大小（解压后），用于防止超大请求/zip bomb导致内存暴涨
	constant.MaxRequestBodyMB = GetEnvOrDefault("MAX_REQUEST_BODY_MB", 128)
	constant.AIImageMaxMemoryMB = GetEnvOrDefault("AI_IMAGE_MAX_MEMORY_MB", 4096)
	constant.AIImageMaxConcurrency = GetEnvOrDefault("AI_IMAGE_MAX_CONCURRENCY", 12)
	...
	// GenerateDefaultToken 是否生成初始令牌，默认启用。
