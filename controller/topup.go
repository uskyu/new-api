	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		defer UnlockOrder(verifyInfo.ServiceTradeNo)
		{
			...
			err = model.DB.Transaction(func(tx *gorm.DB) error {
				if err := model.LockForUpdate(tx).Where("trade_no = ?", verifyInfo.ServiceTradeNo).First(&topUp).Error; err != nil {
					return err
				}
				...
				if topUp.PaymentMethod != verifyInfo.Type {
					logger.LogInfo(c.Request.Context(), fmt.Sprintf("epay actual payment method differs trade_no=%s order_payment_method=%s actual_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, topUp.PaymentMethod, verifyInfo.Type, c.ClientIP()))
				...
				if quotaToAdd <= 0 {
					return fmt.Errorf("invalid epay topup quota: trade_no=%s amount=%d", topUp.TradeNo, topUp.Amount)
				}
			...
			if processed {
				logger.LogInfo(c.Request.Context(), fmt.Sprintf("epay topup success trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), quotaToAdd, topUp.Money, common.GetJsonString(topUp)))
