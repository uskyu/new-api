...
type InactiveUserSummary struct {
	...
	NeverCalledCount     int64   `json:"never_called_count"`
	NeverCalledBalance   int64   `json:"never_called_balance"`
...
func GetInactiveUserAnalytics(pageInfo *common.PageInfo, options InactiveUserAnalyticsOptions) (*InactiveUserAnalytics, error) {
	...
	trackingStartedAt := getUserLastAPIActivityTrackingStartedAt()
	...
	inactive := base.
		...
		Where(`((COALESCE(u.last_api_activity_at, 0) > 0 AND u.last_api_activity_at <= ?)
			OR (COALESCE(u.last_api_activity_at, 0) = 0 AND ? <= ?))`, cutoff, trackingStartedAt, cutoff)
	...
	var summary struct {
		...
		NeverCalledCount     int64 `gorm:"column:never_called_count"`
		NeverCalledBalance   int64 `gorm:"column:never_called_balance"`
	}
	if err := inactive.Session(&gorm.Session{}).Select(`COUNT(*) AS inactive_user_count,
		COALESCE(SUM(u.quota), 0) AS inactive_balance_quota,
		COALESCE(SUM(CASE WHEN COALESCE(u.last_api_activity_at, 0) = 0 THEN 1 ELSE 0 END), 0) AS never_called_count,
		COALESCE(SUM(CASE WHEN COALESCE(u.last_api_activity_at, 0) = 0 THEN u.quota ELSE 0 END), 0) AS never_called_balance`).
		Scan(&summary).Error; err != nil {
		return nil, err
	}

	items := make([]InactiveUserAnalyticsRow, 0)
	if err := inactive.Session(&gorm.Session{}).
		Select(`u.id, u.username, u.display_name, u.quota, u.used_quota, u.created_at,
			COALESCE(u.last_api_activity_at, 0) AS last_api_activity_at, u.inviter_id,
		...
		Order("COALESCE(u.last_api_activity_at, 0) ASC").
		Order("u.quota DESC").
		Order("u.id DESC").
	...
	for i := range items {
		lastActivityAt := items[i].LastAPIActivityAt
		if lastActivityAt <= 0 {
			lastActivityAt = trackingStartedAt
			if items[i].CreatedAt > lastActivityAt {
	...
	return &InactiveUserAnalytics{
		...
		Summary: InactiveUserSummary{
			...
			NeverCalledCount:     summary.NeverCalledCount,
			NeverCalledBalance:   summary.NeverCalledBalance,
