...
func TestStreamScannerHandler_CopiesRepeatedCodexHeaders(t *testing.T) {
	...
	resp.Header.Add("X-Reasoning-Included", "summary")
	resp.Header.Add("X-Reasoning-Included", "encrypted")
	resp.Header.Add("X-Codex-Turn-State", "turn-1")
	resp.Header.Add("X-Codex-Turn-State", "turn-2")
	resp.Header.Add("X-Codex-Turn-State", "")

	StreamScannerHandler(c, resp, info, func(data string, sr *StreamResult) {})

	assert.Equal(t, []string{"summary", "encrypted"}, c.Writer.Header().Values("X-Reasoning-Included"))
	assert.Equal(t, []string{"turn-1", "turn-2"}, c.Writer.Header().Values("X-Codex-Turn-State"))
}
